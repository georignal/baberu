...
app.patch('/api/cards/:id', async (req: any, res: any) => { await db.updateCard(req.params.id, req.body); res.json(await db.getCard(req.params.id)); });
app.delete('/api/cards/:id', async (req: any, res: any) => { await db.deleteCard(req.params.id); res.json({ message: 'Deleted' }); });
app.get('/api/cards/:id/source', async (req: any, res: any) => {
  const card = await db.getCard(req.params.id);
  if (!card) { res.status(404).json({ error: 'Card not found' }); return; }
  const doc = await db.getDocument(card.documentId); const sentences = await db.getSentences(card.documentId);
