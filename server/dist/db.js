...
const empty = { users: [], documents: [], segments: [], sentences: [], vocabulary: [], occurrences: [], cards: [], reviewLogs: [], reviewStates: [] };
let fileStore;
...
function saveFileStore() {
    try {
        if (!fs.existsSync(DATA_DIR))
            fs.mkdirSync(DATA_DIR, { recursive: true });
        fs.writeFileSync(DATA_FILE, JSON.stringify(fileStore, null, 2), 'utf-8');
    }
    catch { }
}
loadFileStore();
...
export const db = {
    async upsertUser(username) {
        ...
        const e = fileStore.users.find((u) => u.username === username);
        if (e)
            return e;
        const u = { id: uuid(), username, createdAt: now() };
        fileStore.users.push(u);
        saveFileStore();
        return u;
    },
    async createDocument(data) {
        const doc = { id: uuid(), userId: currentUserId, title: data.title, fileType: data.fileType || 'text', text: data.text, createdAt: now(), updatedAt: now() };
        if (usingSupabase) {
            await supabase.from('documents').insert({ id: doc.id, user_id: doc.userId, title: doc.title, file_type: doc.fileType, text: doc.text });
        }
        else {
            fileStore.documents.push(doc);
            saveFileStore();
        }
        return doc;
    },
    async listDocuments() {
        if (usingSupabase) {
            ...
            return (data || []).map((d) => ({ id: d.id, userId: d.user_id, title: d.title, fileType: d.file_type, text: d.text, createdAt: d.created_at, updatedAt: d.updated_at }));
        }
        return fileStore.documents.filter((d) => d.userId === currentUserId);
    },
    async getDocument(id) {
        ...
        return fileStore.documents.find((d) => d.id === id);
    },
    async deleteDocument(id) {
        if (usingSupabase)
            await supabase.from('documents').delete().eq('id', id);
        else {
            fileStore.documents = fileStore.documents.filter((d) => d.id !== id);
            saveFileStore();
        }
    },
    ...
    async getSegments(docId) {
        if (usingSupabase) {
            ...
            return (data || []).map((d) => ({ id: d.id, documentId: d.document_id, paragraphIndex: d.paragraph_index, text: d.text, startOffset: d.start_offset, endOffset: d.end_offset }));
        }
        return fileStore.segments.filter((s) => s.document_id === docId).map((d) => ({ id: d.id, documentId: d.document_id, paragraphIndex: d.paragraph_index, text: d.text, startOffset: d.start_offset, endOffset: d.end_offset }));
    },
    ...
    async getSentences(docId) {
        if (usingSupabase) {
            ...
            return (data || []).map((d) => ({ id: d.id, documentId: d.document_id, segmentId: d.segment_id, sentenceIndex: d.sentence_index, text: d.text, startOffset: d.start_offset, endOffset: d.end_offset }));
        }
        return fileStore.sentences.filter((s) => s.document_id === docId).map((d) => ({ id: d.id, documentId: d.document_id, segmentId: d.segment_id, sentenceIndex: d.sentence_index, text: d.text, startOffset: d.start_offset, endOffset: d.end_offset }));
    },
    ...
    async createOccurrence(data) {
        ...
        if (usingSupabase)
            await supabase.from('vocabulary_occurrences').insert({ id: r.id, vocabulary_id: r.vocabularyId, document_id: r.documentId, segment_id: r.segmentId, sentence_id: r.sentenceId, surface_text: r.surfaceText, start_offset: r.startOffset, end_offset: r.endOffset });
        return r;
    },
    async getOccurrences(docId) {
        if (usingSupabase) {
            const { data } = await supabase.from('vocabulary_occurrences').select('*').eq('document_id', docId);
            return (data || []).map((d) => ({ id: d.id, vocabularyId: d.vocabulary_id, documentId: d.document_id, segmentId: d.segment_id, sentenceId: d.sentence_id, surfaceText: d.surface_text, startOffset: d.start_offset, endOffset: d.end_offset }));
        }
        return [];
    },
    async createCard(data) {
        const card = { id: uuid(), vocabularyId: data.vocabularyId, documentId: data.documentId, sentenceId: data.sentenceId, frontText: data.frontText, reading: data.reading, meaning: data.meaning, partOfSpeech: data.partOfSpeech, exampleSentences: data.exampleSentences, dictData: data.dictData, status: 'new', createdAt: now(), updatedAt: now() };
        if (usingSupabase) {
    ...
    async getCard(id) {
        ...
        return fileStore.cards.find((c) => c.id === id);
    },
    async updateCard(id, data) {
        if (usingSupabase)
            await supabase.from('cards').update(data.status ? { status: data.status } : {}).eq('id', id);
        else {
            const c = fileStore.cards.find((c) => c.id === id);
            if (c) {
                if (data.status)
    ...
    async deleteCard(id) {
        if (usingSupabase)
            ...
            fileStore.cards = fileStore.cards.filter((c) => c.id !== id);
            saveFileStore();
        }
    },
    async createReviewLog(cardId, result) {
        ...
        if (usingSupabase)
            await supabase.from('review_logs').insert({ id: r.id, user_id: currentUserId, card_id: cardId, result });
        else {
            fileStore.reviewLogs.push(r);
            saveFileStore();
        }
        return r;
    },
    async getTodayReviews() {
        ...
        return fileStore.reviewLogs.filter((r) => r.reviewedAt.startsWith(today));
    },
    ...
    async getReviewState(cardId) {
        ...
        return fileStore.reviewStates.find((r) => r.cardId === cardId);
    },
    async upsertReviewState(cardId, result) {
        ...
        if (usingSupabase) {
            ...
            const e = fileStore.reviewStates.find((r) => r.cardId === cardId);
            const n = new Date();
            if (e) {
                const stage = result === 'again' ? 0 : Math.min((e.stage || 0) + 1, intervals.length - 1);
                const days = intervals[stage];
                ...
                e.stage = stage;
                e.intervalDays = days;
                e.dueAt = due.toISOString();
                e.lastReviewedAt = n.toISOString();
            }
            else {
                const days = result === 'again' ? 1 : intervals[0];
                const due = new Date(n);
    ...
    async getDueCards() {
        ...
        if (usingSupabase) {
            ...
            const reviewedSet = new Set((reviewedIds || []).map((d) => d.card_id));
            const newIds = (allIds || []).filter((c) => !reviewedSet.has(c.id)).map((c) => c.id);
        ...
        const dueIds = fileStore.reviewStates.filter((r) => r.dueAt && r.dueAt <= now2).map((r) => r.cardId);
        const reviewedIds = fileStore.reviewStates.map((r) => r.cardId);
        const newIds = fileStore.cards.filter((c) => !reviewedIds.includes(c.id)).map((c) => c.id);
        return fileStore.cards.filter((c) => dueIds.includes(c.id) || newIds.includes(c.id));
    },
