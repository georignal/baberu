...
router.post('/:id/parse', async (req, res) => {
    ...
    for (const seg of parsed.segments) {
        const segRecord = segments.find((s) => s.paragraph_index === seg.paragraphIndex);
...
router.post('/:id/extract-vocabulary', async (req, res) => {
    const doc = await db.getDocument(req.params.id);
    if (!doc) {
        res.status(404).json({ error: 'Document not found' });
        return;
...
router.get('/:id/vocabulary-candidates', async (req, res) => {
    const doc = await db.getDocument(req.params.id);
    if (!doc) {
        res.status(404).json({ error: 'Document not found' });
        return;
    ...
    const result = occurrences.map((occ) => {
        ...
        return { id: occ.id, word: occ.surfaceText, reading: '', lemma: occ.surfaceText, partOfSpeech: '', meaning: '', frequency: occurrences.filter((o) => o.vocabularyId === occ.vocabularyId).length, sentenceId: occ.sentenceId, sentenceText: sent?.text || '', documentId: occ.documentId, segmentId: occ.segmentId };
    });
