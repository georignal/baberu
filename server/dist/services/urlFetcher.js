...
export async function fetchUrlText(url) {
    ...
    if (!res.ok) {
        ...
        if (ct.includes('application/pdf'))
            throw new Error('PDF files are not supported. Please copy-paste the text instead.');
        throw new Error(`Server returned ${res.status} ${res.statusText}`);
    }
    const html = await res.text();
    if (!html || html.length < 100)
        throw new Error('Page is empty or too short');
    let root;
    try {
        root = parse(html);
    }
    ...
    if (!text || text.length < 10)
        throw new Error('Could not extract text. The page may be mostly images, require login, or use JavaScript rendering.');
