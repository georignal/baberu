...
for (const [kanji, kana, meaning] of BUILTIN_DICT) {
  if (!BUILTIN_MAP.has(kanji)) BUILTIN_MAP.set(kanji, meaning);
  if (!BUILTIN_MAP.has(kana)) BUILTIN_MAP.set(kana, meaning);
}
...
function init(): Promise<void> {
  ...
  initPromise = new Promise((resolve, reject) => {
    kuromoji.builder({ dicPath: DIC_PATH }).build((err: Error | null, tok: typeof tokenizer) => {
      if (err) {
        initPromise = null;
        reject(err);
        return;
...
function getMeaning(lemma: string, surface: string): string {
...
export function getDictEntry(word: string): import('./dictionary.js').LookupResult | null {
  return lookup(word);
}
 ...
 * 歩い + て + いる → 歩いている
 */
export function mergePhrases(tokens: Token[]): Token[] {
  ...
  while (i < tokens.length) {
    ...
    if (CONTENT.has(t.partOfSpeech) && i + 1 < tokens.length) {
      ...
      while (j < tokens.length && ATTACHABLE.has(tokens[j].partOfSpeech)) j++;
 ...
 */
export function deduplicateTokens(tokens: Token[]): Array<Token & { frequency: number }> {
  const map = new Map<string, { token: Token; count: number }>();
