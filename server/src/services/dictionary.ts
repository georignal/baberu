...
interface DictSense {
  ...
  x?: { jp: string; en: string }[];  // example sentences (from JMdict examples)
}
...
async function load(): Promise<void> {
  ...
  try {
    ...
    const entries: [string, DictEntry][] = JSON.parse(raw);
    ...
    for (const [kanji, kana, meaning, pos] of BUILTIN_DICT) {
      ...
      if (!index.has(kanji)) index.set(kanji, entry);
      if (!index.has(kana)) index.set(kana, entry);
    }
